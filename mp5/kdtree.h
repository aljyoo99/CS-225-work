...
class KDTree
{
  ...
  private:
	...
	/** This is a helper function that finds the nearest point to the query among a group of points stored in a KDTree. */
	Point<Dim> findNearestHelper(const Point<Dim>& query, KDTreeNode* root, int dim, double& radius) const;
	...
	bool possibleOtherSide(const Point<Dim>& query,const Point<Dim>& curr, int dimension, double distance) const;
	...
	/** This helper function helps build the KD Tree. */
	void buildTree(int start, int end, int dimension, KDTreeNode *& node);
